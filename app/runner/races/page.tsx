...
async function getRacesPageData() {
  ...
  // Get all races
  const { data: allRaces } = await supabase.from("races").select("*").order("date", { ascending: true })
  ...
  const upcomingRaces =
    (allRaces || []).filter((race) => {
      ...
      return raceDate >= now || race.status === "upcoming" || race.status === "current"
    }) || []

  const pastRaces =
    (allRaces || []).filter((race) => {
      const raceDate = new Date(race.date)
      return raceDate < now && race.status === "past"
    }) || []

  // Get user's teams
  const { data: teams } = await supabase.from("teams").select("*").or(`manager_id.eq.${user.id},members.cs.{${user.id}}`)

  // Get registrations for user's teams
  let registrations: Registration[] = []
  if (teams && teams.length > 0) {
    const teamIds = teams.map((t) => t.id)
    const { data: regs } = await supabase.from("registrations").select("*").in("team_id", teamIds)
